...
| Variable | Definition | Type |
|---|---|---|
| `explainability_or_feature_selection_method` | Method(s) reported for explainability or predictor-level reporting; formal XAI (SHAP/SAGE) and predictor-level/feature-selection methods (Gini, Boruta, SESv, permutation importance, LASSO-based selection, etc.) listed; "No formal XAI..." indicates no formal XAI even where feature-selection methods were used; "None" or "Not reported" indicates neither was reported — semicolon-separated when multiple | Text |
| `implementation_form` | None / nomogram / web calculator / risk score / EMR integration concept | Categorical |
| `readiness_level` | L1 (concept only) / L2 (internal-only prototype) / L3 (prototype with temporal or external validation) / L4 (real-world deployment) | Categorical |
