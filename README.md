...
│   ├── full_text_assessed_template.csv          # item-level template
│   └── excluded_full_text_with_reasons_template.csv  # item-level template
│
├── 03_data_extraction/
...
│   ├── Figure1_PRISMA_v3.png
│   ├── figure1_prisma_script_v3.py
...
Open `02_screening_records/prisma_flow_data.csv` to verify the screening flow numerically; cross-check with `Figure1_PRISMA_v3.png`. Per-stage exclusion counts and reasons are documented in `prisma_flow_data.csv` and in §2.3 of the manuscript. Item-level templates (`full_text_assessed_template.csv`, `excluded_full_text_with_reasons_template.csv`) document the schema used during screening; item-level records are retained by the authors and available upon reasonable request.
...
- **Figures:** Figures 1–5 prepared using diagrammatic and Excel-based tools from the source data tables in `05_figures_and_tables_source/`. Figure 1 (PRISMA flow) was finalized in Python (matplotlib); the script (`figure1_prisma_script_v3.py`) and source CSV are provided. Figure 6 (best-model AUROC by algorithm class) was prepared in Python (matplotlib) from a CSV input; the script (`figure6_script.py`) and source CSV are provided.
