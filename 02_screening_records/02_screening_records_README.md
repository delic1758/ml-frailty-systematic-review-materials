...
| `ieee_acm_screening_log.xlsx` | Complete record-level screening log for the supplementary IEEE Xplore + ACM Digital Library search conducted during peer review. Four sheets: Summary (search dates, filters, PRISMA stages, Bertini construct-validity justification, references), IEEE (18 records with full authors, journal/volume, sample, population, classification, eligibility decision), ACM (24 records with DOI and venue), Legend. | 42 |
...
## Supplementary IEEE Xplore + ACM Digital Library search
...
- `ieee_acm_fulltext_excluded.xlsx` — the seven full-text candidates with per-record exclusion reasons. Reasons span sample size <1,000 (Amjad 2025, Eskandari 2022, Minici 2022, Ozaki 2024), age criterion not met (Olugbenga 2025), search-record duplicate of a primary-search record (Jung 2021; same paper as primary-search record R23, which had already been excluded at full-text assessment for sample size below 1,000 [n = 74], and therefore not a new eligible study), and construct-validity mismatch (Bertini 2018).

### Supplementary-search PRISMA flow

The supplementary search retrieved 42 records (IEEE 18 + ACM 24). No records were removed before title/abstract screening. All 42 records were screened at the title/abstract stage; 35 were excluded at title/abstract screening (including one record that was both a systematic review and a search-record duplicate of a previously screened primary-search record), and 7 were assessed at full-text eligibility. All 7 were excluded: 4 for sample size <1,000, 1 for age criterion not met, 1 search-record duplicate of a primary-search full-text exclusion, and 1 for construct-validity mismatch. The supplementary search therefore yielded **0 new inclusions**; the original 14-study inclusion is unchanged. Figure 1 was revised to document the supplementary IEEE Xplore and ACM Digital Library search, with record-level decisions provided in Supplementary File S8 / `ieee_acm_screening_log.xlsx` and Supplementary Table S7 / `ieee_acm_fulltext_excluded.xlsx`.
